...
#        pragma clang diagnostic ignored                                       \
            "-Wformat" // warning: format specifies type 'int' but the argument
...
#        pragma clang diagnostic ignored                                       \
            "-Wimplicit-int-float-conversion" // warning: implicit conversion
                                              ...
                                              // precision
#        pragma clang diagnostic ignored                                       \
            "-Wunsafe-buffer-usage" // warning: 'xxx' is an unsafe pointer used
                                    // for buffer access
#        pragma clang diagnostic ignored                                       \
            "-Wnontrivial-memaccess" // warning: first argument in call to
                                     // 'memset' is a pointer to non-trivially
                                     // copyable type
#        pragma clang diagnostic ignored                                       \
...
#        pragma GCC diagnostic ignored                                         \
            "-Wformat" // warning: format '%p' expects argument of type
                       // 'int'/'void*', but argument X has type 'unsigned
                       // int'/'ImGuiWindow*'
#        pragma GCC diagnostic ignored                                         \
...
#        pragma GCC diagnostic ignored                                         \
            "-Wstrict-overflow" // warning: assuming signed overflow does not
                                // occur when simplifying division / ..when
...
#        pragma GCC diagnostic ignored                                         \
            "-Wconversion" // warning: conversion to 'xxxx' from 'xxxx' may
                           // change value
#        pragma GCC diagnostic ignored                                         \
            "-Wsign-conversion" // warning: conversion to 'xxxx' from 'xxxx' may
                          ...
                          const char *format, ImGuiSliderFlags flags)
{
    ...
    const bool is_already_past_limits_and_pushing_outward = is_bounded
        && !is_wrapped
                       ...
                       const char *format, ImGuiSliderFlags flags)
{
    ...
    const char *value_buf_end = value_buf
        + DataTypeFormatString(value_buf, IM_COUNTOF(value_buf), data_type,
                               p_data, format);
    if (g.LogEnabled)
        LogSetNextTextDecoration("{", "}");
...
#    define IMSTB_TEXTEDIT_GETPREVCHARINDEX IMSTB_TEXTEDIT_GETPREVCHARINDEX_IMPL
                                                    ...
                                                    int idx)
    {
        ...
        unsigned int c;
        return idx
            + ImTextCharFromUtf8(&c, obj->TextSrc + idx,
                                 obj->TextSrc + obj->TextLen);
    }
...
#    define STB_TEXTEDIT_MOVEWORDRIGHT STB_TEXTEDIT_MOVEWORDRIGHT_IMPL
    ...
    // Reimplementation of
    // stb_textedit_move_line_start()/stb_textedit_move_line_end() which
...
void ImGuiInputTextState::OnKeyPressed(int key)
{
    ...
    if (key_u == STB_TEXTEDIT_K_LEFT || key_u == STB_TEXTEDIT_K_LINESTART
        ...
        || key_u == STB_TEXTEDIT_K_BACKSPACE
                        ...
                        void *callback_user_data)
{
    ...
    if (is_multiline)
    {
        ImVec2 backup_pos = window->DC.CursorPos;
        ItemSize(total_bb, style.FramePadding.y);
        bool no_clip = (g.InputTextDeactivatedState.ID == id)
            || (g.ActiveId == id)
    ...
    const bool input_requested_by_user = (user_clicked)
        || (g.ActiveId == 0 && (flags & ImGuiInputTextFlags_TempInput));
    const ImGuiID scrollbar_id = (is_multiline && state != NULL)
        ? GetWindowScrollbarID(draw_window, ImGuiAxis_Y)
        : 0;
    const bool user_scroll_finish = is_multiline && state != NULL
        && g.ActiveId == 0 && g.ActiveIdPreviousFrame == scrollbar_id;
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        const bool is_paste =
            ...
            && !is_readonly;
        const bool is_undo =
            (Shortcut(ImGuiMod_Ctrl | ImGuiKey_Z, f_repeat, id)) && !is_readonly
            && is_undoable;
        const bool is_redo =
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_selection)
        {
            ...
            const float bg_offy_up = is_multiline
                ? 0.0f
                : -1.0f; // FIXME-DPI: those offsets should be part of the
                         // style? they don't play so well with multi-line
                         // selection.
            ...
            const char *text_selected_begin = buf_display
                + ImMin(state->Stb->select_start, state->Stb->select_end);
            const char *text_selected_end = buf_display
                + ImMax(state->Stb->select_start, state->Stb->select_end);
    ...
    if (render_cursor)
    {
        ...
        if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
            draw_window->DrawList->AddLineV(
                ...
                1.0f
                    * (float)(int)style
                          ._MainScale); // FIXME-DPI: Cursor thickness (#7031)
                       ...
                       ImGuiColorEditFlags flags)
{
        ...
        && (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        const bool hide_prefix = draw_color_marker
            || (w_per_component
    ...
    else
    {
        is_open =
            storage->GetInt(storage_id,
                            (flags & ImGuiTreeNodeFlags_DefaultOpen) ? 1 : 0)
            != 0;
    }
...
static void TreeNodeStoreStackData(ImGuiTreeNodeFlags flags, float x1)
{
    ...
    const bool draw_lines = (flags
        ...
        != 0;
    tree_node_data->DrawLinesX1 = draw_lines
        ? (x1 + g.FontSize * 0.5f + g.Style.FramePadding.x)
        : +FLT_MAX;
                         ...
                         ImGuiMultiSelectFlags ms_flags)
{
    ...
    bs->EndPosRel = WindowPosAbsToRel(
        ...
        ImClamp(g.IO.MousePos, scope_rect.Min,
                scope_rect.Max)); // Clamp stored position according to current
                                  // scrolling view
...
static ImRect CalcScopeRect(ImGuiMultiSelectTempData *ms, ImGuiWindow *window)
{
    ...
    else
    {
        ...
        scope_rect.Min = ImMin(
            scope_rect.Min
                + ImVec2(window->DecoInnerSizeX1, window->DecoInnerSizeY1),
            scope_rect.Max);
        return scope_rect;
    }
}
                    ...
                    if (table->CurrentColumn != -1)
                    {
                        ...
                        if (clip_min_x
                            != clip_max_x) // When zero sized we expect that
                                           // bounds have been clamped and thus
                                           // are unreliable
                        {
                            item_rect.Min.x =
...
bool ImGui::BeginMenuEx(const char *label, const char *icon, bool enabled)
{
    ...
    ImGuiWindowFlags window_flags = ImGuiWindowFlags_ChildMenu
...
#    endif
    ...
    {
        const bool visible_without_hover = unsaved_marker_visible
