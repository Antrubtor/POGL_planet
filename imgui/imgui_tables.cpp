...
#    endif
    ...
    if (inner_window != outer_window)
    {
        ...
        inner_window->DC.NavLayersActiveMask |= 1
            << table->NavLayer; // So empty table don't appear to navigate
                                // differently.
    ...
    if (unfreeze_rows_request)
    {
        ...
        if (unfreeze_rows_actual)
        {
            ...
            table->RowPosY2 = window->DC.CursorPos.y = table->WorkRect.Min.y
                + table->RowPosY2 - table->OuterRect.Min.y;
                                    ...
                                    int data_count)
{
    ...
    const ImVec2 header_angled_vector = unit_right
    ...
    const float ascent_scaled = g.FontBaked->Ascent
                                     ...
                                     bool enabled)
{
    ...
    const bool has_been_moved = (g.ActiveId == id)
    ...
    bool ret =
        Selectable(label, false, selectable_flags, ImVec2(min_w, label_size.y))
        ...
        const bool is_reordering =
            (g.ActiveId != 0 && g.ActiveIdWindow == g.CurrentWindow
             ...
             && g.ActiveIdHasBeenEditedBefore); // FIXME: This is a bit of a
                                                // hack.
        const int reorder_src_order = is_reordering
            ? table->Columns[table->ReorderColumn].DisplayOrder
            : -1;
        const int reorder_min_order = is_reordering
