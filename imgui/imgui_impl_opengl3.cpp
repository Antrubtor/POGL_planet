...
//  2026-04-23: OpenGL: Added support for standard draw callbacks (in
//  platform_io): DrawCallback_ResetRenderState, DrawCallback_SetSamplerLinear,
//  DrawCallback_SetSamplerNearest. (#9378) 2026-03-12: OpenGL: Fixed invalid
//  assert in ImGui_ImplOpenGL3_UpdateTexture() if ImTextureID_Invalid is
//  defined to be != 0, which became the default since 2026-03-12. (#9295)
...
#        pragma clang diagnostic ignored                                       \
            "-Wcast-function-type" // warning: cast between incompatible
...
#        define IMGUI_IMPL_OPENGL_MAY_HAVE_POLYGON_MODE // may have
                                                        // glPolygonMode()
#    endif
...
struct ImGui_ImplOpenGL3_Data
{
    ...
    GLuint
        NextSampler; // Used if !HasBindSampler && UseTexParameterToSetSampler.
...
#    ifdef IMGUI_IMPL_OPENGL_MAY_HAVE_BIND_SAMPLER
    if (bd->HasBindSampler)
        glBindSampler(
            0, bd->TexSamplers[0]); // We use combined texture/sampler state.
                                    // Applications using GL 3.3 and GL ES 3.0
    ...
    for (const ImDrawList *draw_list : draw_data->CmdLists)
    {
        ...
        for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                // writing using glTextParameter() if sampler is ever changed
                // explicitly.
